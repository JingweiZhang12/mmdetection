...
class VideoValLoop(ValLoop):
    ...
    def run_iter(self, idx, data_batch: Sequence[dict]):
        ...
        if len(track_data_sample) == ori_video_len:
            ...
            if frame_id == 0:
                video_outputs = track_data_sample.new()
            elif frame_id == ori_video_len - 1:
                self.evaluator.process(
                    data_samples=video_outputs, data_batch=data_batch)
            else:
                video_outputs.video_data_samples.append(track_data_sample[0])
                video_outputs.video_length += 1
