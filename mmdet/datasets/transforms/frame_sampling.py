...
class UniformSample(BaseTransform):
    ...
    def sampling_frames(self, video_length, key_frame_id=None):
        """"""
        if key_frame_id is None:
            key_frame_id = random.sample(list(range(video_length)), 1)
        ...
        if video_length > 1:
            ...
            if len(valid_ids) < self.num_ref_img:
                valid_ids = valid_ids * self.num_ref_img
            ref_frame_ids = random.sample(valid_ids, self.num_ref_img)
        else:
            ref_frame_ids = [key_frame_id] * self.num_ref_img

        sampled_frames_ids = [key_frame_id] + ref_frame_ids
        sampled_frames_ids = sorted(sampled_frames_ids)

        # get the index of the key and reference frames
        # in the sorted sampled frames
        key_frames_inds = sampled_frames_ids.index(key_frame_id)
        ref_frames_inds = list(range(0, key_frames_inds)) + list(
            range(key_frames_inds + 1, len(sampled_frames_ids)))
        key_frames_inds = [key_frames_inds]
        return sampled_frames_ids, key_frames_inds, ref_frames_inds
    ...
    def transform(self, video_infos: dict) -> Optional[Dict[str, List]]:
        ...
        (sampled_frames_ids, key_frames_inds,
         ref_frames_inds) = self.sampling_frames(
             video_infos['video_length'], key_frame_id=key_frame_id)
        results = self.prepare_data(video_infos, sampled_frames_ids)
        results['key_frames_inds'] = key_frames_inds
        results['ref_frames_inds'] = ref_frames_inds

        return results

    def __repr__(self) -> str:
        repr_str = self.__class__.__name__
        repr_str += f'(num_ref_img={self.num_ref_img}, '
